...
import { Search, X, Car, ChevronDown } from "lucide-react";
...
export function GarageCarList({ cars, preferences, selectedCarId, carImageBase }: Props) {
  ...
  const selectedDisplayName =
    preferences[selectedCarId] ?? slugToName(selectedCarId);
    ...
    <>
      ...
      <div className="lg:hidden">
        <div className="relative">
          <select
            value={selectedCarId}
            onChange={(e) => router.push(`/garage?car=${e.target.value}`)}
            className={cn(
              "w-full appearance-none rounded-lg border border-border bg-card",
              "pl-4 pr-10 py-3 text-sm font-medium text-foreground",
              "focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary",
              "transition-colors cursor-pointer"
            )}
          >
            {cars.map((c) => {
              const name = preferences[c.car_id] ?? slugToName(c.car_id);
              return (
                <option key={c.car_id} value={c.car_id}>
                  {name} · {c.sessions} {t("summary.sessionsOther")}
                </option>
              );
            })}
          </select>
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5 px-1">
          {cars.length} {cars.length === 1 ? t("summary.carsOne") : t("summary.carsOther")} {t("summary.driven")}
        </p>
      </div>

      {/* ── Desktop: sidebar com busca e lista ─────────────────────────── */}
      <div className="hidden lg:flex w-56 shrink-0 flex-col gap-2">
        <div className="relative">
